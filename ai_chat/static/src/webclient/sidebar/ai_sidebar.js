...
export class AiSidebar extends Component {
    ...
    setup() {
        ...
        onMounted(async () => {
            let chatHistory = await this.aiChatService.getHistory();
            this.state.chatHistory = chatHistory;
            this.createAiChat();
            if (this.aiChat && this.aiChat.addMessages) {
                this.aiChat.addMessages(this.state.chatHistory);
            }
        });
    ...
    createAiChat() {
        ...
        this.aiChat = window.createAiChat()
            .withAdapter(adapter)
            .withDisplayOptions({
                colorScheme: theme
            })
